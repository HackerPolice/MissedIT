...
void AnimFix::FrameStageNotify(ClientFrameStage_t stage)
{
    ...
    animstate->lastAnimUpdateTime = std::max(0.0f, globalVars->curtime - animstate->lastAnimUpdateFrame); // negative values possible when clocks on client and server go out of sync..
