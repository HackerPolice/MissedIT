...
static void DrawBulletTracers(CUserCmd *cmd)
{
	...
	if (isteammate && engine->GetLocalPlayer() != ESP::bulletBeam.enemyIndex &&
	    !Settings::ESP::FilterAlise::BulletBeam::enabled) {
		return;
	} else if (!isteammate && !Settings::ESP::FilterEnemy::BulletBeam::enabled) {
		return;
	} else if (engine->GetLocalPlayer() == ESP::bulletBeam.enemyIndex &&
	...
	for (float i = 0.01f; i <= shite; i += 0.01) {
		debugOverlay->DrawPill(entity->GetEyePosition(), ESP::bulletBeam.bulletPosition.at(0), i, color.Value.x * 255,
		                       color.Value.y * 255, color.Value.z * 255, 100, 10);
	}
	...
	} else if (player != localplayer) // then it means it is our friend nigga
	{
		...
		if (Settings::ESP::FilterAlise::playerInfo::enabled) {
			DrawPlayerText(player, localplayer, x, y, w, h);
	           ...
	           Settings::ThirdPerson::toggled) // There is no need to check this must I try to be secure xd
	{
