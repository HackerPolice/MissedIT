...
static void AutoShoot(C_BaseCombatWeapon* activeWeapon, CUserCmd* cmd, LegitWeapon_t* currentSettings)
{
	if (!activeWeapon || activeWeapon->GetInReload())
		return;
	if (currentSettings->autoScopeEnabled && Util::Items::IsScopeable(*activeWeapon->GetItemDefinitionIndex()) && !Legitbot::localplayer->IsScoped() && !(cmd->buttons & IN_ATTACK2) && !(cmd->buttons&IN_ATTACK) )
	{
...
void Legitbot::CreateMove(CUserCmd* cmd)
{
    ...
    if (!Legitbot::localplayer || !Legitbot::localplayer->IsAlive())
		return;
    activeWeapon = (C_BaseCombatWeapon*)entityList->GetClientEntityFromHandle(Legitbot::localplayer->GetActiveWeapon());
	...
	if (currentWeaponSetting->ignoreJumpEnabled && (!(Legitbot::localplayer->GetFlags() & FL_ONGROUND) && Legitbot::localplayer->GetMoveType() != MOVETYPE_LADDER))
		return;
	...
	if (player)
	{
		...
		{	
			if (currentWeaponSetting->predEnabled)
				Aimbot::VelocityExtrapolate(player, bestSpot); // trying to assume next position of the player
			...
			if (currentWeaponSetting->errorMarginEnabled)
			{
				...
				if ((Legitbot::localplayer->GetShotsFired() > lastShotFired)) //get new random spot when firing a shot or when aiming at a new target
