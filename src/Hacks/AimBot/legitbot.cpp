...
void Legitbot::GetClosestSpot(C_BasePlayer *localplayer, C_BasePlayer *enemy, Vector &BestSpot,
...
C_BasePlayer *
Legitbot::GetClosestPlayerAndSpot(CUserCmd *cmd, C_BasePlayer *localplayer, bool visibleCheck, Vector &bestSpot,
                                  float &bestDamage, const LegitWeapon_t &currentSettings)
{
...
bool Legitbot::IsInFov(C_BasePlayer *localplayer, C_BasePlayer *player, const Vector &spot,
                       const LegitWeapon_t &weaponSettings)
{
...
void Legitbot::AimStep(C_BasePlayer *player, QAngle &angle, CUserCmd *cmd, bool &shouldAim,
                       const LegitWeapon_t &currentSettings)
{
...
void Legitbot::AutoShoot(C_BasePlayer *localplayer, C_BaseCombatWeapon *activeWeapon, CUserCmd *cmd,
                         LegitWeapon_t *currentSettings)
{
	...
	if (currentSettings->autoScopeEnabled && Util::Items::IsScopeable(*activeWeapon->GetItemDefinitionIndex()) &&
	    !localplayer->IsScoped() && !(cmd->buttons & IN_ATTACK2) && !(cmd->buttons & IN_ATTACK)) {
...
void Legitbot::CreateMove(CUserCmd *cmd)
{
	...
	C_BasePlayer *player = Legitbot::GetClosestPlayerAndSpot(cmd, localplayer, true, bestSpot, bestDamage,
	                                                         *currentWeaponSetting);
