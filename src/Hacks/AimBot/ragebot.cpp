...
bool Ragebot::canShoot(C_BaseCombatWeapon *activeWeapon, Vector &bestSpot, C_BasePlayer *enemy,
...
void Ragebot::GetDamageAndSpots(C_BasePlayer *player, Vector &Spot, int &Damage, const int &playerHelth, int &i,
                                const std::unordered_map<int, int> *modelType)
{
	...
	switch (static_cast<DesireBones>(i)) {
		case DesireBones::BONE_HEAD:
			...
			if (currentWeaponSetting->desiredMultiBones[i]) {
				HitboxHead(
						boneID); // lamda expression because again creating a new method is going to make the source code mess :p
	...
	for (int i = 1; i < maxClient; ++i) {
		...
		for (boneIndex = 0; boneIndex < 6; boneIndex++) {
			...
			if (bestDamage >= playerHelth) {
				...
				if (currentWeaponSetting->DamageOverride > 0 && bestDamage >= currentWeaponSetting->DamageOverride &&
				    bestDamage > BestDamage) {
					BestDamage = bestDamage;
					BestSpot = bestSpot;
					enemy = player;
...
void
Ragebot::AutoShoot(C_BasePlayer *player, C_BasePlayer *localplayer, C_BaseCombatWeapon *activeWeapon, CUserCmd *cmd,
                   Vector &bestspot, QAngle &angle, RageWeapon_t *currentSettings)
{
