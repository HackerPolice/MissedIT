...
void Eventlog::Paint()
{
	...
	if (!Settings::Eventlog::showEnemies && !Settings::Eventlog::showTeammates &&
	    !Settings::Eventlog::showLocalplayer) {
		return;
	}
...
void Eventlog::FireGameEvent(IGameEvent *event)
{
	...
	if (strstr(event->GetName(), XORSTR("player_hurt"))) {
		...
		if ((engine->GetPlayerForUserID(hurt_player_id) == engine->GetLocalPlayer() &&
		     (engine->GetPlayerForUserID(attacker_id) != engine->GetLocalPlayer()))) {
		...
		} else if ((engine->GetPlayerForUserID(hurt_player_id) != engine->GetLocalPlayer() &&
		            (engine->GetPlayerForUserID(attacker_id) == engine->GetLocalPlayer()))) {
			damageLog += XORSTR(" to ");
		...
		C_BasePlayer *buyer_player = (C_BasePlayer *) entityList->GetClientEntity(
				engine->GetPlayerForUserID(buyer_player_id));
		if (!buyer_player) {
			return;
		}

		if ((localplayer != buyer_player) && Entity::IsTeamMate(buyer_player, localplayer) &&
		    !Settings::Eventlog::showTeammates) {
			return;
		}

		if ((localplayer != buyer_player) && !Entity::IsTeamMate(buyer_player, localplayer) &&
		    !Settings::Eventlog::showEnemies) {
			return;
		}
		...
		C_BasePlayer *bomb_player = (C_BasePlayer *) entityList->GetClientEntity(
				engine->GetPlayerForUserID(bomb_player_id));
		if (!bomb_player) {
			return;
		}

		if ((localplayer != bomb_player) && Entity::IsTeamMate(bomb_player, localplayer) &&
		    !Settings::Eventlog::showTeammates) {
			return;
		}
		...
		C_BasePlayer *defuse_player = (C_BasePlayer *) entityList->GetClientEntity(
				engine->GetPlayerForUserID(defuse_player_id));
		if (!defuse_player) {
			return;
		}

		if ((localplayer != defuse_player) && Entity::IsTeamMate(defuse_player, localplayer) &&
		    !Settings::Eventlog::showTeammates) {
			return;
		}
		...
		C_BasePlayer *planter_player = (C_BasePlayer *) entityList->GetClientEntity(
				engine->GetPlayerForUserID(plant_player_id));
		if (!planter_player) {
			return;
		}

		if ((localplayer != planter_player) && Entity::IsTeamMate(planter_player, localplayer) &&
		    !Settings::Eventlog::showTeammates) {
			return;
		}
		...
		if ((engine->GetPlayerForUserID(pickup_player_id) == engine->GetLocalPlayer()) &&
		    !Settings::Eventlog::showLocalplayer) {
			return;
		}

		C_BasePlayer *pickup_player = (C_BasePlayer *) entityList->GetClientEntity(
				engine->GetPlayerForUserID(pickup_player_id));
		if (!pickup_player) {
			return;
		}

		if ((localplayer != pickup_player) && Entity::IsTeamMate(pickup_player, localplayer) &&
		    !Settings::Eventlog::showTeammates) {
			return;
		}
		...
		if ((engine->GetPlayerForUserID(drop_player_id) == engine->GetLocalPlayer()) &&
		    !Settings::Eventlog::showLocalplayer) {
			return;
		}

		C_BasePlayer *drop_player = (C_BasePlayer *) entityList->GetClientEntity(
				engine->GetPlayerForUserID(drop_player_id));
		if (!drop_player) {
			return;
		}

		if ((localplayer != drop_player) && Entity::IsTeamMate(drop_player, localplayer) &&
		    !Settings::Eventlog::showTeammates) {
			return;
		}
		...
		if (strstr(event->GetString(XORSTR("item")), XORSTR("knife")) ||
		    strstr(event->GetString(XORSTR("item")), XORSTR("vesthelm"))) {
			return;
		}
